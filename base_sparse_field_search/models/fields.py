...
def _get_attrs(self, model, name):
    ...
    if attrs.get('sparse') and attrs.get('allow_search'):
...
def parse(self):
    ...
    for leaf in self.stack:
        ...
        if field and hasattr(field, 'sparse') and field.sparse:
            field.store= True
            test = True
            fields_to_unstore.append(field)
    parse.super(self)
    for field_unstore in fields_to_unstore:
        field_unstore.store = False
...
def _leaf_to_sql(self, eleaf):
    ...
    if hasattr(model._fields[left], 'sparse') and model._fields[left].sparse:
        ...
        # Special case for boolean, we need to cast in order to work...
...
fields.Serialized.column_type = ('jsonb', 'jsonb')
